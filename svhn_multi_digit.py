...
def conv_graph(images):
    ...
    with tf.name_scope('conv5'):
        ...
        variable_summary(weights.name, weights)
        variable_summary(biases.name, biases)
        conv4 = tf.nn.relu(tf.nn.conv2d(pool4, weights, strides=[1, 1, 1, 1], padding='SAME', name='conv') + biases, name='relu')
        activation_summary(conv4.name, conv4)
        pool5 = tf.nn.max_pool(conv4, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool')
        num_conv_pool += 1
