"""Evaluates MNIST multi-digit model performance.
...
on the inference graph of the MNIST multi-digit model.
...
# Checkpoint directory of trained MNIST multi-digit model
cp_dir = 'saved_models/mnist_multi_digit/'


def main(argv):
    # Get MNIST multi-digit test dataset
    ...
    with tf.Graph().as_default() as graph:
        ...
        for _ in range(100):  # 100 batches
            ...
            # Obtain predicted length and digits by running graph with batch
            # inputs
            length_pred, digits_pred = sess.run(
                [length_logits, digits_logits], feed_dict=feed_dict)

            # Workout predicted length from the lenght logits
            length = np.argmax(length_pred, axis=1)

            # Workout predicted digits from the digits logits
            digits = []
            ...
            for i in range(BATCH_SIZE):
                ...
                # Append actual and predicted labels into
                # 'title' for the image
                title = 'A:' + A + ' ' + 'P:' + P
                ...
                if A != P:
                    ...
                    plt.imshow(np.reshape(
                        images[i], [IMAGE_HEIGHT, IMAGE_WIDTH]))
                    plt.axis('off')
                    plt.title(title)
                    # Save each image
                    plt.savefig(
                    ...
                    fig_savepath = 'eval/mnist_multi_digit/positive_samples'
